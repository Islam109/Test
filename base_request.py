...
# GET существующего питомца
pet_info = base_request.get('pet', 1)
pprint.pprint(pet_info)

# POST - обновление имени существующего питомца
data = {'name': 'Barsic'}
pet_id = base_request.post('pet', 1, data)

# Проверяем, что имя обновилось
pet_info = base_request.get('pet', 1)
assert data['name'] == pet_info['name'], f"Expected {data['name']}, but got {pet_info['name']}"
print("POST assertion passed!")

# DELETE - удаление питомца
request_id = base_request.delete('pet', 1)

# Пытаемся получить удаленного питомца (ожидаем ошибку)
pet_info = base_request.get('pet', 1, expected_error=True)
pprint.pprint(pet_info)